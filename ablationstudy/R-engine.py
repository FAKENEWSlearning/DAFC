...
def train_test(model, train_loader, val_loader, criterion, epochs, dataset):
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            loss_imoe[0][0].backward(retain_graph=True)
            loss_imoe[0][1].backward(retain_graph=True)
            loss_imoe[0][2].backward(retain_graph=True)
            loss_imoe[0][3].backward(retain_graph=True)
            # loss_aca_image (imoe_layer)
            #print("loss_imoe", loss_imoe)
            #print("loss_imoe[1][0]", loss_imoe[1][0])
            loss_imoe[1][0].backward(retain_graph=True)
            loss_imoe[1][1].backward(retain_graph=True)
            loss_imoe[1][2].backward(retain_graph=True)
            loss_imoe[1][3].backward(retain_graph=True)
            # loss_aca_multi (imoe_multi_layer)
            loss_imoe_multi[0].backward(retain_graph=True)
            loss_imoe_multi[1].backward(retain_graph=True)
            loss_imoe_multi[2].backward(retain_graph=True)
            loss_imoe_multi[3].backward(retain_graph=True)
            ...
            # 10
            train_running_loss_aca_text1 += loss_imoe[0][0].item() * labels.size(0) + loss_imoe[0][2].item() * labels.size(0)
            train_running_loss_aca_text2 += loss_imoe[0][1].item() * labels.size(0) + loss_imoe[0][3].item() * labels.size(0)
            train_running_loss_aca_image1 += loss_imoe[1][0].item() * labels.size(0) + loss_imoe[1][2].item() * labels.size(0)
            train_running_loss_aca_image2 += loss_imoe[1][1].item() * labels.size(0) + loss_imoe[1][3].item() * labels.size(0)
            train_running_loss_imoe_multi1 += loss_imoe_multi[0].item() * labels.size(0) + loss_imoe_multi[2].item() * labels.size(0)
            train_running_loss_imoe_multi2 += loss_imoe_multi[1].item() * labels.size(0) + loss_imoe_multi[3].item() * labels.size(0)
        ...
        loss_imoe_text_avg = [
            train_running_loss_aca_text1 / train_num,
            train_running_loss_aca_text2 / train_num
        ]
        loss_imoe_image_avg = [
            train_running_loss_aca_image1 / train_num,
            train_running_loss_aca_image2 / train_num
        ]
        loss_imoe_multi_avg = [
            train_running_loss_imoe_multi1 / train_num,
            train_running_loss_imoe_multi2 / train_num
        ]
