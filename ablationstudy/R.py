...
class Model(nn.Module):
    def __init__(self, dataset, input_dim=1024, hidden_dim=512, output_dim=386):
        ...
        # blip_t_i_m0 --> blip_t_f, blip_v_f
        self.blip_t_i_m0x = nn.Sequential(
            nn.Linear(self.model_dim*2, self.model_dim), 
            nn.BatchNorm1d(self.model_dim), 
            #nn.ReLU()
            nn.LeakyReLU(0.1),
        ...
        # blip_i_t_m0 --> blip_v_f, blip_t_f
        self.blip_i_t_m0x = nn.Sequential(
            nn.Linear(self.model_dim*2, self.model_dim), 
            nn.BatchNorm1d(self.model_dim), 
            #nn.ReLU()
            nn.LeakyReLU(0.1),
        ...
        # blip_t_i --> blip_t_i_m0, blip_i_t_m0
        self.blip_t_ix = nn.Sequential(
            nn.Linear(self.model_dim*2, self.model_dim), 
            nn.BatchNorm1d(self.model_dim), 
            #nn.ReLU()
            nn.LeakyReLU(0.1),
    ...
    def forward(self, images, input_ids, attention_mask,token_type_ids, pre_images, pre_text, labels, feature_mode="clip"):
       ...
       # print("image shape", v.shape)
        ...
        # t_f0 = self.t_fx(t_f)
        t_f0 = t_f
        # blip_t_f0 = self.blip_t_fx(blip_t_f)
        blip_t_f0 = blip_t_f
        # v_f0 = self.v_fx(v_f)
        v_f0 = v_f
        # blip_v_f0 = self.blip_v_fx(blip_v_f)
        blip_v_f0 = blip_v_f

        # blip_t_i_m0 --> blip_t_f, blip_v_f
        x = torch.cat([blip_t_f, blip_v_f], dim=1)
        blip_t_i_m0 = self.blip_t_i_m0x(x)
        # blip_i_t_m0 --> blip_v_f, blip_t_f
        y = torch.cat([blip_t_f, blip_v_f], dim=1)
        blip_i_t_m0 = self.blip_i_t_m0x(y)
        # blip_t_i --> blip_t_i_m0, blip_i_t_m0
        z = torch.cat([blip_t_i_m0, blip_i_t_m0], dim=1)
        blip_t_i = self.blip_t_ix(z)
        # t_i_m0 --> t_f, v_f
        m = torch.cat([t_f, v_f], dim=1)
        t_i_m0 = self.t_i_m0x(m)
        # i_t_m0 --> v_f, t_f
        n = torch.cat([v_f, t_f], dim=1)
        i_t_m0 = self.i_t_m0x(n)
        # t_i --> t_i_m0, i_t_m0
        p = torch.cat([t_i_m0, i_t_m0], dim=1)
        t_i = self.t_ix(p)
        ...
        mohloss_text_text_blip =0 
        mohloss_blip_text_text =0
        mohloss_image_image_blip = 0
        mohloss_blip_image_image = 0
        mohloss_blip_text_image = 0
        mohloss_text_image = 0
        mohloss_blip_image_text = 0
        mohloss_image_text = 0
