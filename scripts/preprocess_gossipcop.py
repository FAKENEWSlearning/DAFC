...
# 图像存储目录
IMAGES_DIR = './data/gossipcop_images'
# 输出文件
OUTPUT_FILE = './data/gossipcop_dataset/gossipcop_news_data_cleaned.txt'
...
def process_and_save():
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as outfile:
        # 1. 遍历类别文件夹 (gossipcop_fake, gossipcop_real)
        for category_folder in os.listdir(DATASET_ROOT):
            if 'gossipcop_' not in category_folder:
                continue
            ...
            for news_folder in os.listdir(category_path):
                ...
                try:
                    with open(json_path, 'r', encoding='utf-8') as f:
                        ...
                        top_img_url = news_folder.split('-')[-1]
...
def split_dataset():
    ...
    # 3. 保存为新的文件
    train_df.to_csv('./data/gossipcop_dataset/gossipcop_train_tweets.txt', sep='|', index=False, header=False)
    test_df.to_csv('./data/gossipcop_dataset/gossipcop_test_tweets.txt', sep='|', index=False, header=False)
